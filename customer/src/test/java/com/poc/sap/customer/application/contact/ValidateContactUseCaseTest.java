...
class ValidateContactUseCaseTest {
    ...
    void recordsValidatingThenTargetTransition() {
        ...
        verify(stateRepo).beginCycle(eq("customer"), eq("C-1:CONTACT"), any(SyncStateTransition.class));
        verify(stateRepo, times(1)).transition(eq("customer"), eq("C-1:CONTACT"), any(SyncStateTransition.class));
